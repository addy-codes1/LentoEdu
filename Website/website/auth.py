...
from .models import User,Responses
...
def questionnaire():
    if request.method == 'POST':      
        ...
        response = Responses(
            ...
            q5=q5,
            q6=q6,
            user_id=current_user.id  # Associate the response with the current user
        )
        # response.save_to_db()

        db.session.add(response)
        db.session.commit()
        flash('respoise received!', category='success')
        for response in current_user.responses:
            print(f"Response: {response.q1}, {response.q2}, {response.q3}, {response.q4}, {response.q5}, {response.q6}")
        print("done")
